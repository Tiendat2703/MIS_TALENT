...
8. (OUTPUT_SCHEMA) Có tách `triggered_rule_approval_required` khỏi
   `manual_evidence_review_required` không? Nếu còn evidence gap có tính chặn thì
   `risk_assessment_status=INCOMPLETE`, `overall_risk_level=null` và có
   `review_priority`; không được gọi null là NONE. Riêng
   `RR-007:reference_date` là gap không chặn vì Team Pack không có ngày tham chiếu
   nghiệp vụ: evaluation vẫn `INSUFFICIENT_EVIDENCE`, nhưng assessment có thể
   `COMPLETE` và gap này không tự yêu cầu manual review.
...
10. (AUTHORITY_BOUNDARY — cổng thật) Khi còn precheck approval PENDING, Decision Card
    ...
    - approval đang `pending`/`rejected` mà `decision_status != review` → tự chốt khi
      chưa được người duyệt. HOLD/NEED_MORE_DATA luôn giữ `decision_status=review`;
      request precheck không đảo ngược review và vẫn cần human approval trước khi
      gọi ngân hàng.
...
- Nếu RR-003 hoặc RR-007 triggered, Decision phải giải thích rule và có hành động
  pricing/cost hoặc operations review tương ứng. Không được ép mọi hợp đồng ACTIVE
  thành `RECOMMEND_HOLD`; các option quản
...
Với case KHÔNG ACTIVE, `RECOMMEND_HOLD` hoặc `NEED_MORE_DATA` không được chặn tạo
precheck request khi đã có amount, funding type và sản phẩm hợp lệ. Validator phải
PASS request PENDING này nếu score/note vẫn null và chưa có API nào được thực thi
trước human approval. `RR-007:reference_date` đơn lẻ là gap không chặn và không được
dùng để đổi APPROVE/APPROVE_WITH_CONDITION thành HOLD.
