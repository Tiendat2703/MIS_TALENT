...
gọi lại. Luồng API đề xuất:
    ...
    1. POST /runs                      -> start_validated_pipeline_run(...) trả session_id ngay
...
``start_validated_pipeline_run`` cấp session_id TRƯỚC rồi chạy pipeline có cổng
Validator ở task nền, nên client mở được SSE ngay từ event đầu tiên. State
(event_bus, task registry) nằm in-memory trong tiến trình — hợp với dashboard live
một tiến trình; nếu scale nhiều worker thì cần chuyển event_bus sang backend chia sẻ
(Redis...).
"""
...
async def start_pipeline_run(
    *,
    ...
    """Chạy pipeline legacy không có cổng Validator ở task nền.

    Các API public dùng ``start_validated_pipeline_run``; hàm này chỉ được giữ cho
    tương thích với các caller nội bộ cũ.
    """
