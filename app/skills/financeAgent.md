...
- Handoff phải giữ hai nhánh canonical tách biệt: `finance_details.contract_finance`
  chỉ chứa số liệu của hợp đồng/order/invoice/funding need; `portfolio_context` chỉ
  chứa thanh khoản toàn OPC. Không đưa cash/reserve gap danh mục vào
  `contract_finance`, kể cả trong diễn giải.
